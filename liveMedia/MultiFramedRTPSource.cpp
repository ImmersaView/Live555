...
void MultiFramedRTPSource::doGetNextFrame1() {
  while (fNeedDelivery) {
	...
	envir() << "MultiFramedRTPSource::doGetNextFrame1(): The total received frame size exceeds the client's buffer size ("
		...
		LIVELOG_ERROR("[Live:ReadTruncated],Client's buffer size exceeded; data will be dropped,bufferSize,%d,dropped,%d", fSavedMaxSize, fNumTruncatedBytes);
      }
...
::getNextCompletedPacket(Boolean& packetLossPreceded) {
  ...
  if (timeThresholdHasBeenExceeded) {
    // ST - log this seqno/loss
    LIVELOG_WARN("[Live:Loss],LOSS! Expected %05d, skipping to %05d (skipped %d)", fNextExpectedSeqNo, fHeadPacket->rtpSeqNo(), fHeadPacket->rtpSeqNo() - fNextExpectedSeqNo);
