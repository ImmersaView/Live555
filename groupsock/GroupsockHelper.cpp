...
void SetSendingInterfaceAddr(const char* ipString)
{
    netAddressBits addr = our_inet_addr(ipString);
    if (addr != INADDR_NONE)  // Check IP was valid
    {
        SendingInterfaceAddr = addr;
    }
}

void SetReceivingInterfaceAddr(const char* ipString)
{
    netAddressBits addr = our_inet_addr(ipString);
    if (addr != INADDR_NONE)  // Check IP was valid
    {
