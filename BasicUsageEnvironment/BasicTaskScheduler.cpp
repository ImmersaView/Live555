...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
  SingleStepLockMutex();  // Lock before referring to our read/write sets
  ...
  SingleStepUnlockMutex();  // Unlock before blocking call to select()

  int selectResult = select(fMaxNumSockets, &readSet, &writeSet, &exceptionSet, &tv_timeToDelay);
  SingleStepLockMutex();  // Lock now that we are about to access sessions etc
  if (selectResult < 0) {
#if defined(__WIN32__) || defined(_WIN32)
  ...
  SingleStepUnlockMutex();  // Unlock since we are done!
}
